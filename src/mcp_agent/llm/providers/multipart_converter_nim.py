...
_logger = get_logger("multipart_converter_nim")

# Define type aliases for content blocks
ContentBlock = Dict[str, Any]
NIMMessage = Dict[str, Any]


class NIMConverter:
    """Converts MCP message types to NIM API format."""

    @staticmethod
    def _is_supported_image_type(mime_type: str) -> bool:
        """
        Check if the given MIME type is supported by nim's image API.
    ...
    def convert_to_nim(
        ...
        """
        Convert a PromptMessageMultipart message to NIM API format.
        ...
        for item in multipart_msg.content:
            try:
                if is_text_content(item):
                    ...
                    content_blocks.append(NIMConverter._convert_image_content(item))

                elif is_resource_content(item):
                    block = NIMConverter._convert_embedded_resource(item)
                    if block:
        ...
        if concatenate_text_blocks:
            content_blocks = NIMConverter._concatenate_text_blocks(content_blocks)
    ...
    @staticmethod
    def convert_prompt_message_to_nim(
        message: PromptMessage, concatenate_text_blocks: bool = False
    ) -> Dict[str, str | ContentBlock | List[ContentBlock]]:
        """
        Convert a standard PromptMessage to NIM API format.
        ...
        return NIMConverter.convert_to_nim(multipart, concatenate_text_blocks)

    @staticmethod
    def _convert_image_content(content: ImageContent) -> ContentBlock:
        """Convert ImageContent to nim image_url content block."""
        # Get image data using helper
        image_data = get_image_data(content)

        # NIM requires image URLs or data URIs for images
    ...
    def _convert_embedded_resource(
        ...
        """
        Convert EmbeddedResource to appropriate NIM content block.

        Args:
            resource: The embedded resource to convert

        Returns:
            An appropriate NIM content block or None if conversion failed
        """
        ...
        mime_type = NIMConverter._determine_mime_type(resource_content)

        # Handle different resource types based on MIME type

        # Handle images
        if NIMConverter._is_supported_image_type(mime_type):
        ...
        # Handle PDFs
            ...
            if is_url and uri_str:
                # NIM doesn't directly support PDF URLs, explain this limitation
                return {
                    "type": "text",
                    "text": f"[PDF URL: {uri_str}]\nNIM requires PDF files to be uploaded or provided as base64 data.",
                }
    ...
    def convert_tool_result_to_nim(
        ...
        Returns:
            Either a single nim message for the tool response (if text only),
        ...
        if text_content:
            # Convert text content to nim format
            temp_multipart = PromptMessageMultipart(role="user", content=text_content)
            converted = NIMConverter.convert_to_nim(
                temp_multipart, concatenate_text_blocks=concatenate_text_blocks
            )

            # Extract text from content blocks
            tool_message_content = NIMConverter._extract_text_from_content_blocks(
        ...
        user_message = NIMConverter.convert_to_nim(non_text_multipart)
    ...
    @staticmethod
    def convert_function_results_to_nim(
        results: List[Tuple[str, CallToolResult]],
        concatenate_text_blocks: bool = False,
    ) -> List[Dict[str, Any]]:
        """
        Convert a list of function call results to NIM messages.
        ...
        Returns:
            List of nim API messages for tool responses
        """
        messages = []

        for tool_call_id, result in results:
            converted = NIMConverter.convert_tool_result_to_nim(
                tool_result=result,
                tool_call_id=tool_call_id,
